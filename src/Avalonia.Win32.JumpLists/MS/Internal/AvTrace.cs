        ...
        if (parameters != null && labels != null && labels.Length > 0)
        {
            ...
            for (; i < labels.Length && j < parameters.Length; i++, j++)
            {
                ...
                // the format string explicitely.
                         ...
                         && !(parameters[j] is AvaloniaProperty))
                {
...
#pragma warning restore 56500
    ...
    readonly GetTraceSourceDelegate _getTraceSourceDelegate;
    readonly ClearTraceSourceDelegate _clearTraceSourceDelegate;
    ...
    static bool? _enabledInRegistry = null;

    static readonly char[] FormatChars = new char[] { '{', '}' };
}

internal delegate void AvTraceEventHandler(AvTraceBuilder traceBuilder, object[] parameters, int start);

[SupportedOSPlatform("Windows")]
internal sealed class AvTraceBuilder
{
    ...
    {
        var argstrs = new object[args.Length];
